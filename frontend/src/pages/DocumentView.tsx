...
const DocumentView = () => {
  ...
  const [document, setDocument] = useState(mockDocument); // Start with mock data, will be replaced with API data
  const [question, setQuestion] = useState('');
  const [isQuerying, setIsQuerying] = useState(false);
  const [queries, setQueries] = useState(mockDocument.queries);
  ...
  React.useEffect(() => {
    const fetchDocument = async () => {
      try {
        ...
        setTimeout(() => {
          setDocument({...mockDocument, id: documentId});
          setQueries(mockDocument.queries);
          setIsLoading(false);
        }, 1000);
      } catch (error) {
  ...
  const handleAskQuestion = () => {
    ...
    setTimeout(() => {
      const newQuery = {
        question,
        answer: `This is a simulated answer to your question: "${question}". In a real implementation, this would be generated by the AI model based on the document content.`,
        timestamp: new Date()
      };
        ...
        <div className="container px-6 mx-auto">
          <div className="mb-8">
            ...
            <div className="flex items-center text-sm text-muted-foreground mt-2">
              <span>Uploaded {mockDocument.uploadedDate.toLocaleDateString()}</span>
              <span className="mx-2">•</span>
              <span>Last viewed {mockDocument.lastViewed.toLocaleDateString()}</span>
            </div>
          </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      onClick={() => copyToClipboard(mockDocument.summary)}
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="prose dark:prose-invert max-w-none">
                    ...
                    <div dangerouslySetInnerHTML={{ __html: mockDocument.content.replace(/\n/g, '<br>').replace(/^## (.*$)/gm, '<h2>$1</h2>').replace(/^# (.*$)/gm, '<h1>$1</h1>') }} />
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
                        ...
                        <div key={index} className="border rounded-lg p-4 animate-scale-in">
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <div className="text-xs text-muted-foreground">
                              {query.timestamp.toLocaleTimeString()} {query.timestamp.toLocaleDateString()}
                            </div>
                          </div>
