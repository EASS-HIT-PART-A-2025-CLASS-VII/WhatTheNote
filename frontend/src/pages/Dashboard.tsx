...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "document-deleted",
      handleDocumentDeleted as EventListener,
    );
    return () => {
  ...
  useEffect(() => {
    ...
    const subjectFromUrl = decodeURIComponent(
      urlParams.get("subject") || "All Subjects",
    );

    if (!hasInitializedSubject.current && documents.length > 0) {
      const uniqueSubjects = [
        "All Subjects",
        ...new Set(documents.map((d) => d.subject)),
      ].filter(Boolean);
